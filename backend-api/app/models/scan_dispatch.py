...
class ScanDispatch(Base):
    ...
    # Phase 9: the durable unit of work is a collection, not a control. A row
    # names either a collector or a result, never both, and a row naming neither
    # is the scan's own run_scan row. The CHECK in the migration is what makes
    # that a property of the schema rather than of the writer.
    collector_id: Mapped[str | None] = mapped_column(String(200), nullable=True)
