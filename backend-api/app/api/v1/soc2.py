...
Configuration drift is a third stream, added in Phase 8 and read exactly like
the manual one: last, additively, and with no path into the coverage arithmetic
or the rating. Everything above the drift read renders identically whether or
not the drift feature exists at all.
"""
...
from app.schemas.provenance import PUBLISHABLE_PROVENANCE_FIELDS
...
#
# Phase 9 moved the tuple itself to app.schemas.provenance and applied it to the
# scan detail and results responses too, which had been publishing policy_source
# verbatim on every row. Re-exported here because this name is the one the
# Phase 7 and Phase 8 tests assert against, and because this is still the module
# that documents why the rule exists.
...
async def _drift_stream(
    ...
    """Read-only view of the Phase 8 drift stream. Runs last, fails closed.

    Modelled on ``_approved_manual_evidence`` and for the same reason: it is the
    last read the report performs, so nothing it does can disturb the automated
    reads above it, and an unreadable stream is reported as unknown rather than
    presented as "no drift".

    Three answers, and they are three distinct claims:

    * a run exists - ``available: true`` with that run's counts;
    * no run exists - ``available: false`` with the ordinary note and zeroed
      counts. "No drift run for this scan" is a known state, not an unreadable
      one, and the drift service returns ``None`` for it rather than raising;
    * the stream could not be read - ``available: false`` with the unavailable
      note. Unknown, never "none" and never compliant.

    ``SQLAlchemyError`` is the fault this expects, but the guarantee made here is
    stronger than that one exception class: no failure of the drift read may
    reach the caller, because the automated projection above it is the part of
    this document that must always render.

    Nothing returned here is a rating, and none of these numbers is added to any
    coverage figure.
    """
    try:
        summary = await drift_service.soc2_drift_summary(db, scan_id, user_id)
        if summary is None:
            return Soc2DriftStream(available=False, note=DRIFT_STREAM_NOTE)
        return Soc2DriftStream(available=True, note=DRIFT_STREAM_NOTE, **summary)
    except Exception:  # noqa: BLE001 - a drift fault must not break the report
        return Soc2DriftStream(available=False, note=DRIFT_STREAM_UNAVAILABLE_NOTE)
...
async def get_soc2_report(
    ...
    drift_stream = await _drift_stream(db, scan.id, current_user.id)
