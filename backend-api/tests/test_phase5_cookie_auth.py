...
def test_google_purge_migration_preserves_identity_links():
    ...
    try:
        ...
        # Derive the expected revision from the migration graph rather than
        # pinning a literal. The literal was correct for Phase 5 but silently
        # became a Phase 6 and then a Phase 7 failure; what this test actually
        # cares about is that "upgrade head" landed on the single head, not on
        # which revision happens to be head this phase.
        from alembic.config import Config
        from alembic.script import ScriptDirectory

        scripts = ScriptDirectory.from_config(
            Config(str(helpers.BACKEND / "alembic.ini"))
        )
        assert helpers._versions(url) == set(scripts.get_heads())
        helpers._alembic(url, "upgrade", "head")
