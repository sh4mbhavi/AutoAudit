...
# The chain has moved on: Phase 9 adds scan_dispatch.collector_id on top of this
# revision. Everything below still asserts the Phase 8 revision's own shape and
# guards; only "what does `upgrade head` land on" belongs to the current head.
CURRENT_HEAD = "b9d4e17c6a52"  # pragma: allowlist secret - migration revision
MIGRATION_PATH = (
    BACKEND
    / "alembic"
    / "versions"
...
def migrated(database_url):
    """A database at the current head carrying one user, one connection, three scans."""
    _alembic(database_url, "upgrade", "head")
    assert _versions(database_url) == {CURRENT_HEAD}
