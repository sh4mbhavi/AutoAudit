...
def test_real_api_cancel_delete_and_ownership(database_url, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        # Phase 9: an optional bounded page, over the same real rows. Omitting
        # `limit` must return exactly what it always returned, which the
        # equality assertion further down still proves.
        paged = client.get(f"/scans/{scan_id}/results?limit=1")
        assert paged.status_code == 200, paged.text
        assert len(paged.json()) == 1
        assert paged.json()[0] == rows[0]
        assert paged.headers["X-Total-Count"] == str(len(rows))
        second = client.get(f"/scans/{scan_id}/results?limit=1&offset=1").json()
        assert second == rows[1:2]
        filtered = client.get(f"/scans/{scan_id}/results?limit=5&status_filter=skipped")
        assert filtered.headers["X-Total-Count"] == str(
            sum(1 for row in rows if row["status"] == "skipped")
        )
        assert client.get(f"/scans/{scan_id}/results?limit=0").status_code == 422
        assert client.get(f"/scans/{scan_id}/results?limit=501").status_code == 422
        assert client.get(f"/scans/{scan_id}/results?offset=-1").status_code == 422
        # Every published provenance record, on both shapes, withholds the Rego
        # source text the SOC 2 projection has always withheld.
        assert all("policy_source" not in (row["provenance"] or {}) for row in rows)
        detail = client.get(f"/scans/{scan_id}").json()
        assert all(
            "policy_source" not in (row["provenance"] or {})
