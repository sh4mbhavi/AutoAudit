...
def test_api_task_worker_result_contract(database_url, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        send.assert_called_once_with(
            "worker.tasks.run_scan", args=[scan_id], queue="autoaudit"
        )
        # Round-trip the actual API task envelope through JSON; no live broker needed.
        envelope = json.dumps(
            {"task": send.call_args.args[0], "args": send.call_args.kwargs["args"]}
        )
        script = """
import json, sys
...
with patch.object(registry, "get_collector", return_value=collector), patch.object(graph_client, "GraphClient", return_value=object()), patch.object(tasks.evaluate_control, "delay", tasks.evaluate_control.run):
    tasks.run_scan.run(*message["args"])
"""
