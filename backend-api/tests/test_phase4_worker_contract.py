...
def execute_serialized(task, **kwargs):
    assert set(kwargs) == {"scan_id", "collector_id", "connection_id"}
    assert type(kwargs["scan_id"]) is int and type(kwargs["connection_id"]) is int
    assert type(kwargs["collector_id"]) is str
    content_type, encoding, body = dumps(kwargs, serializer="json")
    assert "synthetic-fixture" not in body
    task.run(**loads(body, content_type, encoding))
...
with patch.object(registry, "get_collector", return_value=collector), patch.object(graph_client, "GraphClient", return_value=object()), patch.object(dispatcher.celery_app, "send_task", side_effect=capture):
    ...
    for name, kwargs in messages:
        assert name == tasks.evaluate_collection.name
        execute_serialized(tasks.evaluate_collection, **kwargs)
"""
