        ...
        for index, state in enumerate(states):
            ...
            for assignment in mutations:
                with pytest.raises(
                    ...
                    await connection.execute(
                        f"UPDATE scan_result SET {assignment} WHERE id=$1", result_id
                    )
            assert (
                await connection.execute(
    ...
    try:
        ...
        for assignment in [
            ...
            with pytest.raises(asyncpg.CheckViolationError, match="scan.*immutable"):
                await connection.execute(f"UPDATE scan SET {assignment} WHERE id=302")
        assert (
            await connection.execute(
...
def test_pending_result_selection_and_identity_cannot_be_rewritten(
    ...
    with pytest.raises(asyncpg.CheckViolationError, match="result.*immutable"):
        asyncio.run(
            _query(
                url, f"UPDATE scan_result SET {assignment} WHERE id=600", execute=True
            )
        )
...
def test_scan_identity_and_connection_attribution_are_immutable(migrated, assignment):
    ...
    with pytest.raises(asyncpg.CheckViolationError, match="scan.*immutable"):
        asyncio.run(
            _query(url, f"UPDATE scan SET {assignment} WHERE id=301", execute=True)
        )
