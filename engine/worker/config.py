...
# Kept identical to backend-api/app/core/config.py: the two services share the
# key ring but not the code, so the ban has to be stated in both.
DEV_ENCRYPTION_KEY = (
...
class WorkerSettings(BaseSettings):
    ...
    # Phase 10: the dispatcher's Prometheus scrape port. 0 disables the server,
    # which is the default so no existing deployment starts listening on a port
    # its network policy has not been told about.
    METRICS_PORT: int = Field(default=0, ge=0, le=65535)
    # The interface the exporter binds. Prometheus scrapes the container over the
    # private network, so 0.0.0.0 inside a container is the working default --
    # but it is stated rather than inherited, so a host-networked deployment can
    # narrow it without patching code.
    METRICS_BIND: str = "0.0.0.0"  # nosec B104 # container-scoped; see comment

    # Encryption key for decrypting credentials
    ENCRYPTION_KEY: str = ""
    # Phase 10 key ring. Retired keys, comma-separated. The worker never
    # encrypts, so it only ever reads: during a rotation window it must be able
    # to decrypt rows the API wrote under either key. Kept byte-identical in
    # meaning to backend-api/app/core/config.py -- the two services share the
    # ring but not the code, so the pair has to be changed together.
    ...
    def validate_runtime_security(self):
        ...
        # A retired key still decrypts stored tenant credentials, so every check
        # applied to the primary applies to the whole ring.
