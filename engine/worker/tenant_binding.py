...
async def verify_sharepoint_tenant(credentials: dict, graph_client_type) -> bool:
    ...
    try:
        ...
        try:
            ...
            # Phase 9 gave GraphClient a pooled transport that must be released.
            # This probe runs once per SharePoint collection group and would
            # otherwise leak a connection pool every time. `release` never
            # raises: a cleanup failure must not turn a verified tenant into an
            # unverified one, which the enclosing except would otherwise do.
