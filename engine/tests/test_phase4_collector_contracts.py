...
from worker.crosswalk import mapping_control_ids
...
def collect_graph(collector_id, routes=None, status=200, raw=None):
    ...
    # Phase 9: the production attempt count, with the wall-clock delay removed.
    # A 429 here must still exhaust bounded retry and end in the same
    # HTTPStatusError, but these are transport-contract assertions and must not
    # pay real backoff for every collector in the table. Retry timing itself is
    # asserted in test_phase9_graph_transport.py.
    client._retry = dataclasses.replace(
        graph_client.DEFAULT_RETRY, base_seconds=0.0, max_total_delay_seconds=0.0
    )
    requests = []
    ...
    with patch(
        ...
        side_effect=lambda *a, **k: real_client(transport=httpx.MockTransport(respond)),
    ):
...
def powershell_client(**mock_kwargs):
    """A stand-in client supporting both the single and the batched call shape.

    Phase 9 lets a collector issue several reviewed operations in one remote
    session. run_operations delegates to the same run_operation mock, so every
    assertion below -- including that the collector passes its own collector id
    as the second positional argument -- holds for both shapes.
    """
    client = type("Client", (), {"run_operation": AsyncMock(**mock_kwargs)})()

    async def run_operations(entries):
        return [
            await client.run_operation(operation_id, collector_id, **params)
            for operation_id, collector_id, params in entries
        ]

    client.run_operations = run_operations
    return client
...
def test_powershell_denied_and_throttled_never_return_assessment(collector_id, status):
    ...
    client = powershell_client(side_effect=error)
    with pytest.raises(httpx.HTTPStatusError):
...
def test_powershell_malformed_evidence_is_rejected(collector_id):
    client = powershell_client(return_value="malformed")
...
def test_powershell_empty_evidence_is_preserved(collector_id):
    client = powershell_client(return_value=None)
    result = asyncio.run(get_collector(collector_id).collect(client))
...
def test_inventory_covers_all_crosswalk_metadata_collectors():
    ...
    # Read from the versioned SOC 2 mapping artifact rather than repeating the
    # population here; the crosswalk is the single source of truth for it.
    ids = mapping_control_ids()
    assert len(ids) == 44
...
def test_powershell_mixed_malformed_records_are_rejected(collector_id):
    client = powershell_client(return_value=[{"Name": "partial"}, None])
    with pytest.raises(ValueError, match="PowerShell"):
        asyncio.run(get_collector(collector_id).collect(client))
...
def test_antiphish_collects_rules_needed_for_assignment_evaluation():
    ...
    client = powershell_client(side_effect=[policy, rule])
    result = asyncio.run(
        get_collector("exchange.protection.anti_phish_policy").collect(client)
    )
    assert result["anti_phish_rules"] == [rule]
    # Phase 9 sends both reads in one batched session; the second entry is still
    # the rules operation under this collector's own id, in this order.
...
def test_powershell_error_records_cannot_be_mistaken_for_policy_records(collector_id):
    client = powershell_client(return_value={"error": "partial collection"})
    with pytest.raises(ValueError, match="PowerShell"):
        asyncio.run(get_collector(collector_id).collect(client))
