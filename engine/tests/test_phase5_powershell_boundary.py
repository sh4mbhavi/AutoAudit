...
def test_registered_powershell_collectors_all_use_allowlisted_operations():
    """Every reviewed call, in either call shape, names literal ids.

    Phase 9 added ``run_operations`` so several reviewed operations can share one
    remote session. The batch shape is walked here too: a batched entry must be a
    literal ``(operation_id, collector_id, params)`` tuple naming this
    collector's own id, so batching cannot become a way to reach an operation a
    collector is not permitted to run, or to build an operation id at runtime.
    """
    import ast
    ...
    for collector_id, collector in DATA_COLLECTORS.items():
        ...
        for node in ast.walk(tree):
            if not (
                ...
                and node.func.attr in ("run_operation", "run_operations")
            ):
                continue
            if node.func.attr == "run_operation":
                op_id, caller_id = [ast.literal_eval(arg) for arg in node.args]
                reviewed.append(
                    (
                        op_id,
                        caller_id,
                        {"Identity": "shared@contoso.com"} if node.keywords else {},
                    )
                )
                continue
            entries = [
            ...
            assert entries, collector_id
            for entry in entries:
                op_node, caller_node, params_node = entry.elts
                assert isinstance(op_node, ast.Constant) and isinstance(
                    op_node.value, str
                ), collector_id
                assert isinstance(caller_node, ast.Constant) and isinstance(
                    caller_node.value, str
                ), collector_id
                assert isinstance(params_node, ast.Dict), collector_id
                # A batched parameter value may be computed, but its NAME must be
                # a literal from the registry's own parameter map.
                names = [ast.literal_eval(key) for key in params_node.keys]
                reviewed.append(
                    (
                        op_node.value,
                        caller_node.value,
                        {name: "shared@contoso.com" for name in names},
                    )
                )
        assert reviewed, collector_id
        assert "run_cmdlet" not in source
        for op_id, caller_id, parameters in reviewed:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "_run_via_service",
        "_run_via_docker",
        # Phase 9 batch entry points. Batching must not become a way past the
        # validation the single-operation paths perform before any I/O.
        "run_operations",
        "_run_batch_via_service",
        "_run_batch_via_docker",
    ],
)
@pytest.mark.parametrize(
...
def test_every_client_execution_path_validates_before_io(
    ...
    with pytest.raises(ValueError):
        ...
        elif path == "run_operations":
            asyncio.run(client.run_operations([(operation, collector, params)]))
        elif path.startswith("_run_batch"):
            asyncio.run(getattr(client, path)([entry]))
        else:
...
def test_a_batch_cannot_smuggle_one_valid_entry_past_an_invalid_one():
    """Validation is per entry, so one bad entry rejects the whole batch."""
    import asyncio
...
def test_a_batch_cannot_mix_two_modules_in_one_session():
    import asyncio
...
def test_the_batched_docker_path_still_refuses_a_certificate_module():
    import asyncio
...
def test_the_batched_docker_path_passes_secrets_by_name_only(monkeypatch):
    import asyncio
    ...
    def run(argv, **kwargs):
        ...
        return Mock(
            returncode=0,
            stdout=_json.dumps([{"index": 0, "data": {"AuditDisabled": False}}]),
        )

    monkeypatch.setattr(powershell_client.subprocess, "run", run)
    assert asyncio.run(client.run_operations([(OP, COLLECTOR, {})])) == [
        {"AuditDisabled": False}
    ]
    assert "sensitive-token" not in " ".join(captured["argv"])
    assert "sensitive-token" not in captured["input"]
    assert captured["env"]["EXO_TOKEN"] == "sensitive-token"
...
# Literal expectations. Deliberately NOT derived from OPERATIONS: the frozenset
# catches any addition or removal, and the mapping regression-locks the exact
# module and command text that reaches a privileged pwsh child.
EXPECTED_OPERATION_IDS = frozenset(
    {
        ...
        "exchange.protection.atp_policy_o365.read",
        ...
        "exchange.protection.malware_filter_policy.read",
        "exchange.protection.safe_attachment_policy.read",
        "exchange.protection.safe_links_policy.read",
        "exchange.protection.teams_protection_policy.read",
        "exchange.transport.external_in_outlook.read",
        "exchange.transport.transport_rules.read",
        "exchange.transport.transport_rules.outbound_spam",
        "compliance.dlp_compliance_policy.read",
        "compliance.label_policy.read",
        "sharepoint.pnp.tenant.read",
    }
)

EXPECTED_OPERATION_COMMANDS = {
    "exchange.authentication.dkim_signing_config.read": (
        "ExchangeOnline",
...
def test_all_operations_construct_only_their_reviewed_module_and_command():
    ...
    assert frozenset(OPERATIONS) == EXPECTED_OPERATION_IDS
    assert {
        operation_id: (operation.module, operation.command)
        for operation_id, operation in OPERATIONS.items()
    } == EXPECTED_OPERATION_COMMANDS
    expected_import = {
        "ExchangeOnline": "Import-Module ExchangeOnlineManagement",
        "Compliance": "Import-Module ExchangeOnlineManagement",
        "SharePointOnline": "Import-Module PnP.PowerShell",
        "Teams": "Import-Module MicrosoftTeams",
    }
    for operation_id, operation in OPERATIONS.items():
        params = (
        ...
        assert expected_import[operation.module] in script
