...
def test_migrated_scan_evaluates_and_freezes_provenance(
    ...
    dispatcher.tick()
    # Phase 9 dispatches one collection per distinct collector. This fixture has
    # exactly one dispatchable control, so it dispatches exactly one collection
    # and nothing else -- stated exactly, not as membership in a set that would
    # hold whatever the orchestrator happened to send.
    assert [name for name, _ in deliveries] == ["worker.tasks.evaluate_collection"]
    for name, delivery in deliveries:
        assert set(delivery["kwargs"]) == {"scan_id", "collector_id", "connection_id"}
        tasks.evaluate_collection.run(**delivery["kwargs"])
