...
def test_task_signature_has_identifiers_only():
    ...
    assert list(inspect.signature(tasks.evaluate_collection.run).parameters) == [
        "scan_id",
        "collector_id",
        "connection_id",
    ]
...
def test_orchestration_serializes_only_identifiers(monkeypatch):
    ...
    # Phase 9 keys the outbox row on the collector, not the result. A collector
    # id is an identifier from the frozen metadata, exactly as a result id is an
    # identifier from the frozen selection: still no evidence, credential or
    # metadata body crosses the boundary.
    assert send.call_args.args == (session, 1)
    assert send.call_args.kwargs == {"collector_id": "entra.roles.cloud_only_admins"}
