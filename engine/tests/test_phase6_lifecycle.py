...
def lifecycle(database, monkeypatch):
    with database.begin() as c:
        ...
        attempts int NOT NULL DEFAULT 0, available_at timestamp NOT NULL DEFAULT now(),
        dispatched_at timestamp, last_error varchar(100), created_at timestamp NOT NULL DEFAULT now())""")
        )
...
def test_dispatcher_recovers_broker_outage_and_sends_identifiers(
    ...
    with database.begin() as c:
        ...
        c.execute(text("UPDATE scan_dispatch SET available_at=now()"))
...
def test_deadline_fails_scan_and_terminalizes_pending(lifecycle, monkeypatch):
    ...
    monkeypatch.setattr(dispatcher, "get_db_session", session)
    with database.begin() as c:
        c.execute(text("UPDATE scan SET deadline_at=now()-interval '1 second'"))
    dispatcher.tick()
    with database.connect() as c:
...
def test_stalled_delivery_exhaustion_fails_scan(lifecycle, monkeypatch):
    ...
    with database.begin() as c:
        c.execute(
            text(
                "UPDATE scan_dispatch SET attempts=100, dispatched_at=now()-interval '1 day', available_at=now()-interval '1 day'"
            )
        )
    dispatcher.tick()
    with database.connect() as c:
...
def test_orphan_initial_and_stale_child_delivery_are_recovered(lifecycle, monkeypatch):
    ...
    with database.begin() as c:
        ...
        assert len(before) == 2
        c.execute(
            text("UPDATE scan_dispatch SET available_at=now()-interval '1 second'")
        )
    send.reset_mock()
    dispatcher.tick()
...
def test_corrupt_metadata_still_terminalizes_on_deadline(lifecycle, monkeypatch):
    ...
    with database.begin() as c:
        c.execute(
            text(
                "UPDATE scan SET metadata_snapshot='[1]'::jsonb, deadline_at=now()-interval '1 second'"
            )
        )
    dispatcher.tick()
    with database.connect() as c:
...
def test_migrated_legacy_pending_rows_finish_without_invented_attribution(
    ...
    if failure in {"reconcile", "scan_deadline_exceeded"}:
        if failure == "scan_deadline_exceeded":
            with sessions() as s:
                s.execute(text("UPDATE scan SET deadline_at=now()-interval '1 second'"))
