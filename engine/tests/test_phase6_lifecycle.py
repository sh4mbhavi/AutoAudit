...
def lifecycle(database, monkeypatch):
    ...
    metadata = {
        "controls": [
            {
                "control_id": str(i),
                "data_collector_id": f"entra.synthetic.{i}",
...
def test_duplicate_orchestration_builds_atomic_outbox(lifecycle, monkeypatch):
    ...
    with database.connect() as c:
        ...
        # Phase 9: one row per distinct collector, not per control. The fixture's
        # two controls name two different collectors, so the count is unchanged
        # and what the count means is what changed.
        assert len(rows) == 2
        assert {r["collector_id"] for r in rows} == {
            "entra.synthetic.1",
            "entra.synthetic.2",
        }
        assert all(r["result_id"] is None for r in rows)
        assert all(r["task_name"] == "worker.tasks.evaluate_collection" for r in rows)
...
def test_dispatcher_recovers_broker_outage_and_sends_identifiers(
    ...
    assert send.call_args.kwargs["kwargs"] in [
        dict(scan_id=1, collector_id=f"entra.synthetic.{i}", connection_id=3)
        for i in (1, 2)
    ]
...
def test_reconciliation_recreates_missing_child_dispatch(lifecycle, monkeypatch):
    ...
    with database.begin() as c:
        c.execute(
            text("DELETE FROM scan_dispatch WHERE collector_id='entra.synthetic.2'")
        )
    dispatcher.tick()
    with database.connect() as c:
        assert (
            c.execute(
                text(
                    "SELECT count(*) FROM scan_dispatch WHERE collector_id IS NOT NULL"
                )
            ).scalar_one()
            == 2
        )
...
def test_orchestrator_rollback_leaves_pending_without_partial_children(
    ...
    def crash_after_first(session, scan_id, result_id=None, **kwargs):
        original(session, scan_id, result_id, **kwargs)
