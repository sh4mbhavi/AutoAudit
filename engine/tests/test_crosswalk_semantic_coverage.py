...
# The Appendix B population is the SOC 2 crosswalk's own control_resolution
# table; it is read from the versioned mapping artifact so there is exactly one
# source of truth for which controls must carry direct semantic cases.
APPENDIX_B_IDS = frozenset(mapping_control_ids())
REQUIRED_CASES = {
    "pass",
    "fail",
    "missing",
...
def collected_crosswalk_evidence():
    ...
    for collector_id in {
        ...
        client = powershell_client(side_effect=response)
