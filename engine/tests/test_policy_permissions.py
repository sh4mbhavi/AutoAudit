...
POLICY_DIR = Path(__file__).resolve().parents[1] / 'policies/cis/microsoft-365-foundations/v6.0.0'
...
@pytest.mark.parametrize(
    'control', [c for c in READY if c['data_collector_id'] in RECONCILED_REQUIREMENTS],
    ids=lambda c: c['control_id'],
)
def test_reconciled_collectors_declare_documented_read_permissions(control):
    assert set(control['requires_permissions']) == RECONCILED_REQUIREMENTS[control['data_collector_id']]
