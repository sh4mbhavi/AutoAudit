...
# The old assertion here was `compliant == false` with the message "No admin
# accounts detected - check collector permissions". A tenant cannot have zero
# administrators, and the message said as much: this is the collector failing,
# not the tenant. It is indeterminate.
test_indeterminate_zero_admin_accounts if {
	result := data.essential_eight.asd_essential_eight.v2025.control_e8_priv_1_1.result with input as {
		"admin_accounts": [],
...
test_indeterminate_admin_accounts_absent if {
	result := data.essential_eight.asd_essential_eight.v2025.control_e8_priv_1_1.result with input as {}
	result.compliant == null
	result.details.evaluation_status == "indeterminate"
}

test_indeterminate_sync_flag_not_boolean if {
	result := data.essential_eight.asd_essential_eight.v2025.control_e8_priv_1_1.result with input as {
		"admin_accounts": [{"userPrincipalName": "admin1@contoso.com", "on_premises_sync_enabled": "false"}],
...
test_indeterminate_collector_error if {
	result := data.essential_eight.asd_essential_eight.v2025.control_e8_priv_1_1.result with input as {
		"admin_accounts": [{"userPrincipalName": "admin1@contoso.com", "on_premises_sync_enabled": false}],
		"total_admin_accounts": 1,
		"collector_error": "insufficient privileges",
	}
	result.compliant == null
	result.details.evaluation_status == "indeterminate"
}
