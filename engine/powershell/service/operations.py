...
# The most reviewed operations one remote session may run. A bound is required:
# a tenant with thousands of shared mailboxes must not be able to make one pwsh
# child hold an unbounded script or an unbounded session.
MAX_BATCH = 25


def validate_entries(entries: Any) -> tuple[str, tuple[Operation, ...]]:
    """Validate a list of batch entries and return its single module.

    Every entry is validated on its own terms -- its own operation id, its own
    collector id, its own parameters -- so batching cannot become a way to run an
    operation under a collector that is not permitted to run it. The list must
    then resolve to exactly ONE module, because the module is what chooses the
    Connect verb, the certificate namespace and the secret environment variables
    a pwsh child is given; two modules in one child would mean two credential
    sets in one child.

    Deliberately without the per-session size bound: a caller may hold a longer
    list and split it into sessions, and each of those sessions is what
    ``validate_batch`` bounds.
    """
    if type(entries) is not list or not entries:
        raise ValueError("A PowerShell batch requires at least one operation")
    operations = []
    for entry in entries:
        if type(entry) is not dict or set(entry) != {
            ...
            raise ValueError("Unexpected or missing PowerShell batch entry fields")
        operations.append(
            validate_operation(
                entry["operation_id"], entry["collector_id"], entry["params"]
            )
        )
    modules = {operation.module for operation in operations}
    if len(modules) != 1:
        raise ValueError("A PowerShell batch must use exactly one module")
    return modules.pop(), tuple(operations)


def validate_batch(entries: Any) -> tuple[str, tuple[Operation, ...]]:
    """One remote session's worth of operations: validated and size-bounded."""
    if type(entries) is list and len(entries) > MAX_BATCH:
        raise ValueError("PowerShell batch exceeds the reviewed maximum size")
