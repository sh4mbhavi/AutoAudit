 ...
 *   coverage figure.
 *
 * `drift_stream` is a third stream on exactly the same terms: reported beside
 * the automated results, never added to a coverage figure and never able to move
 * a rating.
 */

import type { DriftRunStatus } from './drift';
 ...
 *
 * `available: false` is about readability, not emptiness. With the ordinary
 * `note` it means no drift run exists for this scan, which is a known state;
 * with the unavailable note it means the stream could not be read at all, which
 * is unknown and never "no drift". Render the `note` rather than inferring.
 *
 * Show `no_events_means` wherever an empty result is displayed.
 */
export type Soc2DriftStream = {
  available: boolean;
  baseline_id?: number | null;
  baseline_scan_id?: number | null;
  run_id?: number | null;
  run_status?: DriftRunStatus | null;
  configuration_comparable?: boolean | null;
  evaluation_comparable?: boolean | null;
  /** Counts by event class and severity. No control id, fact name or value. */
...
export type Soc2ReportResponse = {
  ...
  /** Additive in Phase 8. Absent on any response rendered before it existed. */
  drift_stream?: Soc2DriftStream | null;
