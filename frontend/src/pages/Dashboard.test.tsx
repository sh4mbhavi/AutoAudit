...
import { getScans, type ScanListItem, type ScanRead } from '../api/client';
...
it('shows persisted assessment and partial coverage on the dashboard', async () => {
  vi.mocked(getScans).mockResolvedValue(partial<ScanListItem[]>([{ id: 1, status: 'completed', semantics_version: 'phase3-v1', selected_count: 5, total_controls: 5, passed_count: 1, failed_count: 0, not_assessable_count: 2, indeterminate_count: 1, error_count: 1, compliance_score: '100', coverage_score: '20' }]));
...
it('labels collection errors separately from failed controls in suggested follow-up', async () => {
  ...
  vi.mocked(getScans).mockResolvedValue(partial<ScanListItem[]>([{ id: 2, status: 'completed', semantics_version: 'phase3-v1', selected_count: 1, total_controls: 1, error_count: 1, compliance_score: null, coverage_score: 0 }]));
  vi.mocked(getScan).mockResolvedValue(partial<ScanRead>({ id: 2, results: [{ control_id: '1.1', status: 'error', message: 'Could not collect evidence' }] }));
  render(<MemoryRouter><Dashboard isDarkMode /></MemoryRouter>);
...
it.each([
  ...
  vi.mocked(getScans).mockResolvedValue(partial<ScanListItem[]>([{ id: 3, status: 'completed', semantics_version: 'phase3-v1', selected_count: selected, total_controls: selected, passed_count: passed, failed_count: failed, compliance_score: score, coverage_score: coverage }]));
  render(<MemoryRouter><Dashboard isDarkMode /></MemoryRouter>);
